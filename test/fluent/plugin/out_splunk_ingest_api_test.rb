...
describe Fluent::Plugin::SplunkIngestApiOutput do
  ...
  it 'should not have an index in the payload' do
    verify_sent_events(conf: 'index my_index') do |batch|
      batch.each do |item|
        expect(item).wont_include :index
      end
    end
  end
  ...
  def verify_sent_events(args = {}, &blk)
    conf = args[:conf] || ''
    ...
    if status == 200
      api_req = stub_successful_api_request.with do |r|
        yield r.body.split(/(?={)\s*(?<=})/).map {|item| JSON.load item}.first
      end
    else
      api_req = stub_failed_api_request status
    end
